...
Some games ship alternative skins (asset sets), for example a seasonal or operator-branded look. Pass **skinId** to load the game with one of them:
...
The value is passed through untouched to the game, which maps it to its skinned assets, so it must match a skin that the game actually ships. If it is left out (or empty), the game receives `skinId: 'default'`, its standard look. Talk to us to find out which skins are available for a game.
